...
import type { SeoRecommendation, RecommendationPriority } from '../types/seo';
...
function SeoRecommendationGroup({
  priority,
  label,
  icon,
  color,
  ...
  return (
    <div style={{
      ...
      <button
        ...
        aria-label={`${label} priority recommendations: ${items.length} items`}
      >
...
export function SeoRecommendations({ recommendations }: SeoRecommendationsProps) {
  ...
  const priorities: Array<{
    ...
    { key: 'medium', label: 'MEDIUM', icon: '🟢', color: '#22c55e' },
    { key: 'low', label: 'LOW', icon: '⚪', color: '#94a3b8' }
  ];
    ...
    <div className="card" style={{
      ...
      {recommendations.length === 0 && (
        <div style={{
          ...
          <div>No issues found - excellent SEO!</div>
        </div>
      )}
    </div>
  );
